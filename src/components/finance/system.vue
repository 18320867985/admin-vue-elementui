<template>
    <div>
        <div style="margin-bottom: 20px">
            <el-select v-model="queryInfo.userType" placeholder="用户类型" class="input-style">
                <el-option
            ...
            <el-select v-model="queryInfo.businessType" placeholder="业务类型" class="input-style">
                <el-option
            ...
            <el-input v-model="queryInfo.name" placeholder="配置名称" class="input-style"></el-input>
            <el-date-picker
        ...
        <el-dialog
            ...
            <div class="add-dialog">
                <div class="line-item">
                    <div class="info-item">
                        <p><span>*</span>用户类型</p>
                        <el-select v-model="addList.userType" placeholder="用户类型" :disabled="isSee" style="flex: 1">
                            <el-option
                                    v-for="item in options"
                                    :key="item.value"
                                    :label="item.label"
                    ...
                    <div class="info-item" style="margin-right: 0">
                        <p><span>*</span>业务类型</p>
                        <el-select v-model="addList.businessType" placeholder="业务类型" :disabled="isSee" style="flex: 1">
                            <el-option
                                    v-for="item in options1"
                                    :key="item.value"
                                    :label="item.label"
                ...
                <div class="line-item">
                    <div class="info-item">
                        <p><span>*</span>配置名称</p>
                        <el-input v-model="addList.name" placeholder="配置名称" maxlength="20" :disabled="isSee" style="flex: 1"></el-input>
                    </div>
                    <div class="info-item" style="margin-right: 0">
                        <p><span>*</span>值</p>
                        <el-input v-model="addList.value" placeholder="值" type="number" :disabled="isSee" style="flex: 1"></el-input>
                    </div>
                </div>
                <div class="line-item">
                    <p>场景描述</p>
...
<script>
    export default {
        ...
        methods: {
            //获取数据
            getData(){
                this.$api.baseSystemList(this.queryInfo).then(res => {
                    // 网络请求畅通
            ...
            //重置按钮
            resetFn(){
                this.queryInfo.userType = '';
                this.queryInfo.businessType = '';
                this.queryInfo.name = '';
            ...
            addInfo(){
                //判断是否有值为空
                if(!this.addList.userType){
                    this.$message.error('请选择用户类型');
                    return false
                }else if(!this.addList.businessType){
                    this.$message.error('请选择业务类型');
                    return false
                }else if(!this.addList.name){
                    this.$message.error('请输入配置名称');
                    return false
                }else if(!this.addList.value){
                    this.$message.error('请输入值');
                    return false
                }
                this.addList.userType = +this.addList.userType;
                this.addList.businessType = +this.addList.businessType;
                console.log(this.addList);
                if(this.titleText === '编辑'){
                    //发送编辑请求
                    this.$api.baseSystemEdit(this.addList).then(res => {
                        // 网络请求畅通
                        // console.log(res.data);
                    ...
                    //发送新增请求
                    this.$api.baseSystemtAdd(this.addList).then(res => {
                        // 网络请求畅通
                        // console.log(res.data);
                        if(res.data.code === 200){
                            this.$message({
